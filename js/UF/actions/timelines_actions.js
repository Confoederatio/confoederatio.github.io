//Initialise functions
{
  ...
  /*
    ...
    arg0_parent_timeline: (String) - Optional. The ID of the parent timeline to split off from. window.actions.initial_timeline by default
    arg1_options: (Object)
  ...
  */
  window.createTimeline = function (arg0_parent_timeline, arg1_options) {
    //Convert from parameters
    var parent_timeline_id = (arg0_parent_timeline) ? arg0_parent_timeline : window.actions.initial_timeline;
    var options = (arg1_options) ? arg1_options : {};

    //Declare local instance variables
    var new_timeline_id = generateRandomID(window.timelines);
    var parent_timeline_array = window.timelines[parent_timeline_id];

    //Create a new timeline with a base action
    window.timelines[new_timeline_id] = [];

    var new_timeline = window.timelines[new_timeline_id];
    var root_action = {};
    ...
    //Return statement
    return (!options.return_key) ? window.timelines[new_timeline_id] : options.return_key;
  }

  /*
    constructTimelineGraph() - Returns an A* compatible graph of window.timelines.
  ...
  */
  window.constructTimelineGraph = function () {
    //Declare local instance variables
    var all_timelines = Object.keys(window.timelines);
    var timeline_graph = {};

    //Iterate over all timelines
    for (var i = 0; i < all_timelines.length; i++) {
      var local_timeline = window.timelines[all_timelines[i]];
  ...
  */
  window.deleteTimeline = function (arg0_timeline_id) {
    //Convert from parameters
    var timeline_id = arg0_timeline_id;

    //Declare local instance variables
    var all_timelines = Object.keys(window.timelines);
    var timeline = window.timelines[timeline_id];
    ...
    if (timeline_id == window.actions.current_timeline)
      window.actions.current_timeline = window.actions.initial_timeline;

    //Delete timeline
    delete window.timelines[timeline_id];

    //If this timeline is the initial timeline, delete window.actions.current_timeline; initialise undo/redo again
    if (timeline_id == window.actions.initial_timeline) {
      delete window.actions.current_timeline;
      delete window.actions.initial_timeline;
  ...
  */
  window.generateTimelineGraph = function (arg0_timeline_id, arg1_options) {
    //Convert from parameters
    var timeline_id = arg0_timeline_id;
    var options = (arg1_options) ? arg1_options : {};
    ...
    var timeline_array = window.timelines[timeline_id];
  ...
  /*
    ...
    Returns: (Number)
  */
  window.getTimelineWidth = function (arg0_timeline_id) {
    //Convert from parameters
    var timeline_id = arg0_timeline_id;
  ...
  /*
    ...
    arg1_options: (Object)
      timeline_index: (Number) - Optional. The index off of which the timeline should split. The last index of the timeline by default
  */
  window.loadTimeline = function (arg0_timeline_id, arg1_options) {
    //Convert from parameters
    var timeline_id = arg0_timeline_id;
    var options = (arg1_options) ? arg1_options : {};

    //Declare local instance variables
    var current_index = returnSafeNumber(window.actions.current_index);
    ...
    var start_key = `${window.actions.current_timeline}-${current_index}`;
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      var local_element = window.timelines[split_key[0]][parseInt(split_key[1])];
    ...
    //Set window.actions.current_timeline, window.actions.current_index
    window.actions.current_timeline = timeline_id;
    window.actions.current_index = returnSafeNumber(options.timeline_index);
  }
  ...
  */
  window.jumpToTimeline = function (arg0_timeline_id) {
    //Convert from parameters
    var timeline_id = arg0_timeline_id;
  ...
  */
  window.redoAction = function () {
    //Declare local instance variables
    var current_index = returnSafeNumber(window.actions.current_index);
    var current_timeline = window.timelines[window.actions.current_timeline];
    ...
    if (current_timeline[current_index + 1]) {
      ...
      global[local_element.redo_function](...local_element.redo_function_parameters);
      local_element.delta_toggle = "undo";
      window.actions.current_index = current_index + 1;
  ...
  */
  window.performAction = function (arg0_options) {
    //Convert from parameters
    ...
    var current_timeline = window.timelines[window.actions.current_timeline];
    var current_index = returnSafeNumber(window.actions.current_index);
    var local_action = window.actions[options.action_id];
    var new_action = {
    ...
    if (current_index != current_timeline.length - 1) {
      var new_timeline = createTimeline(window.actions.current_timeline);

      new_timeline.push(new_action);

      //Set current_timeline; current_index to new_timeline
      window.actions.current_timeline = new_timeline[0].id;
      window.actions.current_index = 1;
    } else {
      current_timeline.push(new_action);

      //Set current_index
      window.actions.current_index = current_timeline.length - 1;
    }
  }
