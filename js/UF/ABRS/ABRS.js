...
//Initialise functions
{
  ...
  */
  window.changeSaveDirectory = function (arg0_path) {
    //Convert from parameters
    var path = arg0_path;

    //Set global varaible
    window.ABRS_directory = path;
  }
  ...
  */
  window.cleanCorruptFiles = function (arg0_options) {
    //Convert from parameters
    ...
    if (!window.ABRS_directory) {
      log.error(`cleanCorruptFiles() - window.ABRS_directory is not defined.`);
      return false;
    }
  ...
  */
  window.cleanEmptyFiles = function (arg0_options) {
    //Convert from parameters
    var options = (arg0_options) ? arg0_options : {};
    ...
    //Guard clause if ABRS_directory is not defined
    if (!window.ABRS_directory) {
      log.error(`cleanEmptyFiles() - window.ABRS_directory is not defined.`);
      return false;
    }

    //Declare local instance variables
    var backup_files = loadDirectory(window.ABRS_directory);

    //Iterate over backup_files and test each one
    for (var i = 0; i < backup_files.length; i++) {
      var local_rawdata = fs.readFileSync(`${window.ABRS_directory}/${backup_files[i]}`);
  ...
  /*
    ...
    arg0_options: (Object)
      log: (Boolean) - Optional. Whether to log the backup array. True by default.
  */
  window.cleanInvalidJSONFiles = function (arg0_options) {
    //Convert from parameters
    var options = (arg0_options) ? arg0_options : {};
  ...
  /*
    ...
    Returns: (String)
  */
  window.cleanEscapeStrings = function (arg0_string) {
    //Convert from parameters
  ...
  /*
    ...
    arg0_options: (Object)
      log: (Boolean) - Optional. Whether to log the backup array. True by default.
  */
  window.loadBackupArray = function (arg0_options) {
    //Convert from parameters
    var options = (arg0_options) ? arg0_options : {};
    ...
    //Guard clause if ABRS_directory is not defined
    if (!window.ABRS_directory) {
      log.error(`loadBackupArray() - window.ABRS_directory is not defined.`);
      return false;
    }

    //Declare backup array
    window.backup_array = fs.readdirSync(window.ABRS_directory);
    window.backup_loaded = false;

    //Sort backup array in chronological order
    backup_array.sort(function(a, b) {
      try {
        return fs.statSync(window.ABRS_directory + a).mtime.getTime() - fs.statSync(window.ABRS_directory + b).mtime.getTime();
      } catch {}
    });
  ...
  /*
    loadMostRecentSave() - Loads the most recent save in the current window.backup_array.
    arg0_options: (Same as loadSave())
  */
  window.loadMostRecentSave = function (arg0_options) {
    //Convert from parameters
    var options = (arg0_options) ? arg0_options : {};

    //Load save once reloading window.backup_array
    loadBackupArray();

    var file_path = `${window.ABRS_directory}/${window.backup_array[window.backup_array.length - 1]}`;
  ...
  window.loadRequirements = function () {
    ...
    window.JSONPack = require("jsonpack");
  }

  /*
    loadSave() - Loads a save from a file into window.main, or initialises a custom function for it.
    ...
    arg1_options: (Object)
      ...
      load_object_function: (String) - Optional. The string variable address for the current load object function. Undefined by default, meaning objects will be loaded into window.main
  */
  window.loadSave = function (arg0_file, arg1_options) {
    //Convert from parameters
  ...
  /*
    ...
    Returns: (String)
  */
  window.returnABRSDateString = function () {
    //Declare local instance Variables
    var d = new Date();
  ...
  /*
    ...
    arg0_options: (Object)
      log: (Boolean) -  Optional. Whether to log the backup array. True by default.
  */
  window.writeDB = function (arg0_file, arg1_options) {
    //Convert from parameters
  ...
  */
  window.writeSave = function (arg0_options) {
    //Convert from parameters
    var options = (arg0_options) ? arg0_options : {};

    //Guard clauses
    if (!window.ABRS_directory) {
      log.error(`writeSave() - window.ABRS_directory is not defined.`);
      return false;
    }
    ...
    //Declare local instance variables
    var file_path = `${window.ABRS_directory}/${returnABRSDateString()}.txt`;
